        ...
        controller: [ '$scope', function($scope) {
            ...
            $scope.renderPage = function(num, callback) {
                $scope.pdfDoc.getPage(num).then(function(page) {
                    ...
                    var parentBlock = angular.element('pdfviewer').parent('div');
                    var parentBlockWidth = parentBlock.width();

                    var currentScale = parseFloat((parentBlockWidth/viewport.width).toFixed(1));

                    canvas.width = parentBlockWidth;
                    canvas.height = viewport.height * currentScale;

                    //change pdf width dependent by parent block width
                    viewport = page.getViewport(currentScale);
